...
const contextTemplate = require('./context-template.js');
...
class TooltipView {
  constructor() {
    ...
    this.listeners = {};
    this.targetObject;
    this.requestButtonStatus = true;
    this.myId = idMappings.indexOf(this);

    // Create element wrapper.
    this.element = require('./requestbutton-element-wrapper')();
  ...
  buttonStatus(){
    return this.requestButtonStatus;
  }

  setParams(params) {

    this.element.el.innerHTML = contextTemplate(params, this);
    document.getElementById(`__codecontextbox-${this.myId}`).addEventListener('mouseup', (e) => {
      // send the input text (use id) to scope
      var a = document.getElementById(`code-codecontextbox-${this.myId}`).value
      console.log(a);
  ...
  reposition(targetObject) {
    ...
    this.element.setContextBoxPosition(_tooltipPosition.x, _tooltipPosition.y);
